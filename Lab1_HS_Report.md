...
Практичні інваріанти коректності:
1. Для однієї пари (`candidateUid`, `phase`, `direction`) в обробнику має бути однозначна реакція.
2. Перехід у наступну фазу можливий лише після двох валідних `IN`.
3. Вузол, що програв (побачив більший `UID`), не ініціює нові фази від свого імені.
...
Рекомендована мінімізація полів за типами:
1. `OUT`: `type`, `candidateUid`, `phase`, `direction`, `ttl`.
2. `IN`: `type`, `candidateUid`, `phase`, `direction`.
3. `ANNOUNCE`: `type`, `winnerUid`, `senderUid`.
4. `REPORT`: `type`, `senderUid`, `winnerUid`, `rounds`, `totalMessages`, `timeMs` (за наявності).

Важливо для стабільності:
1. Вказувати `phase` у `OUT/IN`, щоб відділяти застарілі повідомлення.
2. Використовувати однакові назви полів у всіх вузлах.
3. Логувати події у форматі: `timestamp | node | event | payload`.
...
4. `TimeMs` (опційно):
   - `endTime - startTime` у `Launcher`;
   - вимірюється для всього сценарію.
...
### 5.1 Методика проведення вимірювань
Щоб отримати відтворювані дані:
1. Фіксувати однаковий `basePort` і схему генерації `UID`.
2. Для кожного `N` виконувати щонайменше 3-5 запусків.
3. Зберігати сирі логи та підсумкові `REPORT`.
4. В таблицю заносити середнє або медіану (обрати один підхід і вказати його в тексті).

### 5.2 Шаблон блоку для вставки фактичного запуску
```text
Scenario:
N = ...
UID list = [...]
...
TotalMessages = ...
TimeMs = ... (optional)
Check: WinnerUID == max(UID) -> PASS/FAIL
```
...
## 7. Література
1. Hirschberg D. S., Sinclair J. B. Decentralized extrema-finding in circular configurations of processors. Communications of the ACM, 1980.
2. Lynch N. Distributed Algorithms. Morgan Kaufmann, 1996.
3. Attiya H., Welch J. Distributed Computing: Fundamentals, Simulations, and Advanced Topics. Wiley, 2nd edition.
